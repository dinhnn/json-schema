...
public final class ObjectComparator {
  ...
  private static boolean deepEqualArrays(final JsonArray arr1, final JsonArray arr2) {
    ...
    for (int i = 0; i < arr1.size(); ++i) {
      if (!deepEquals(arr1.getValue(i), arr2.getValue(i))) {
        return false;
      }
    }
    return true;
  ...
  private static boolean deepEqualObjects(final JsonObject jsonObj1, final JsonObject jsonObj2) {
    Set<String> obj1Names = jsonObj1.fieldNames();
    if(!obj1Names.equals(jsonObj2.fieldNames())){
      return false;
    }
    for (String name : obj1Names) {
      if (!deepEquals(jsonObj1.getValue(name), jsonObj2.getValue(name))) {
        return false;
      }
    }
    return true;
